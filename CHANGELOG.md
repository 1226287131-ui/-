...
+ [新增] 视频创作台与无限画布新增 `video-v3` 下游适配，支持固定 `720p`、`4-30` 秒、7 种画幅以及图片、视频、音频多参考素材。
+ [新增] 无限画布视频生成支持选择 `1-15` 条，每条视频独立提交、显示进度与错误，并可展开查看批量结果。
+ [修复] 批量视频的重试、取消和删除不会误中断无关任务或遗留后台轮询。
+ [优化] `video-v3` 使用 OpenAI Video 异步创建/查询协议，任务完成后优先读取公开结果地址；结果地址尚未可用时持续轮询。
+ [新增] 视频创作台新增 `MiniMax-H3-933-1440P-GF` 适配，提供六种固定尺寸、5-15 秒、音频开关和最多 5 张参考图。
+ [修复] MiniMax 视频任务改用专用生成/查询接口，兼容嵌套任务响应并对创建后的短暂 `404` 查询进行重试。
+ [调整] `grok-imagine-1.5-video` 使用 Grok Video multipart 下游协议，支持 7 种画幅、1-15 秒及多参考图原生文件上传；多参考图仅允许 480p/720p。
+ [新增] 视频创作台新增 `video-v2-满血兜底版` 下游适配，自动限制为 `16:9`/`9:16`、15 秒和固定 720p，并按文档省略不支持的请求字段。
+ [新增] 视频创作台支持 video-v1、grok-imagine-1.5-video、video-v2/video-v2-fast 四种下游协议，并按模型限制可用时长、画幅和参考素材。
+ [修复] 视频任务轮询兼容嵌套结果与中转站状态，并为鉴权内容下载、直链保存和媒体元数据读取增加超时，避免画布视频节点长期停留在加载状态。
