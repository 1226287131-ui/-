...
+ [新增] 适配 `video-v2-720P（按秒计费）` 和 `video-v2-480P（按秒计费）`，复用 V2 编辑框及请求参数，固定对应分辨率并支持最多 9 图、3 视频和 3 音频参考素材。
+ [修复] `video-v3` 的 30 秒兼容请求按模型分辨率配置提交，`video-v3-480P（按条计费）` 不再错误提交 `720p`。
+ [新增] 适配 `video-v3-720P（按条计费）` 和 `video-v3-480P（按条计费）`，复用 `video-v3` 请求参数与编辑框，分别固定为 `720p` 和 `480p`，并支持最多 30 张参考图、10 个参考视频和 10 个参考音频。
+ [调整] 合并官方 Infinite Canvas v0.18.0，保留现有多模型视频、本地媒体、节点复制、提示词去重、渠道模型持久化及远程视频优先播放等二开功能。
+ [调整] `video-v3` 编辑框新增 `21:9` 画幅选项，并同步支持复用 V3 适配器的模型。
+ [新增] 适配 `video-v2-特价版` 视频模型，沿用 `video-v2` 请求参数与编辑框，仅支持最多 9 张参考图且不支持参考视频和音频。
+ [调整] `video-v2-特价版` 参考素材上限调整为最多 9 张参考图、3 个参考视频和 3 个参考音频，编辑框与 V2 请求参数同步支持。
+ [新增] 适配 `video-v3-特价版` 视频模型，沿用 `video-v3` 请求参数与编辑框，仅支持最多 9 张参考图且不支持参考视频和音频。
+ [新增] 适配 `video-v3-480p` 视频模型，沿用 `video-v3` 请求参数与编辑框并固定分辨率为 `480p`。
+ [修复] 识别带“限时低价渠道”后缀的 `video-v2-fast`、`video-v2` 和 `video-v3` 分支，并复用对应视频编辑框、请求适配器与轮询逻辑。
+ [调整] `video-v2`、`video-v2-fast` 支持 5-15 秒连续自定义，`video-v3` 支持 4-30 秒连续自定义，并将输入值按模型范围提交 `duration`。
+ [新增] 无限画布图片编辑面板支持复制为新图片节点，并继承原提示词、生成配置和参考素材直连。
+ [调整] 下架源码内置模型，新用户仅显示在渠道 API 拉取并选择的模型，并自动清理旧配置中的完整默认模型包。
+ [修复] 渠道 API 获取并选择的模型刷新后不再因同名历史内置模型清理而丢失。
+ [修复] 无限画布视频结果优先使用可播放的上游 URL，并将生成视频的本地缓存与元数据处理改为后台执行，避免高分辨率视频长时间停留在加载状态。
+ [调整] MiniMax-H3 每个画幅的尺寸预设新增 2K、4K，并在高分辨率请求中同步提交 `aspect_ratio`。
+ [调整] MiniMax-H3 隐藏工作流选择，按参考图和尺寸自动提交 `text-to-video`、`multi-reference` 或 `cf-multi-reference`。
+ [修复] 无限画布 MiniMax-H3 设置摘要按当前画幅显示，避免 `9:16`、`1:1` 等比例因 `2K/4K` 尺寸被误显示为横屏。
+ [调整] `video-v2` 编辑框秒数改为支持 `5s` 至 `15s` 的每秒连续选择，`video-v2-满血兜底版` 继续固定 `15s`。
+ [新增] 无限画布新增 `video-v2-fast` 视频适配器，编辑框与请求参数沿用 `video-v2`。
+ [新增] 无限画布新增 `wan-3.0` 视频适配器，编辑框与请求参数沿用 `video-v3`。
+ [修复] 视频任务在编辑器、画布重试/复制和所有 API 提交入口统一规范 `@图片N`、`@视频N`、`@音频N`，确保上游实际请求可识别参考素材。
+ [修复] 画布生成节点分离原始提示词与上游组装提示词，并清理重试、复制及配置节点中的重复提示词。
...
+ [新增] 视频编辑面板支持复制为新视频节点，并按原顺序继承真实参考素材连线、`@` 引用提示词和生成配置。
+ [新增] MiniMax-H3 支持自动推导及手动选择 `workflow_id`，并为 `cf-*` 工作流提供 2K/4K 超分尺寸设置。
+ [修复] 画布视频任务重试时不再重复追加已组装的上游提示词，避免第二次或多次提交提示词膨胀，并保留 H3 工作流配置。
+ [新增] 无限画布支持框选多个图片、视频或音频节点后，通过一次拖线批量连接到同一目标节点。
