...
export function CanvasNodePromptPanel({ node, nodes, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, onCreateImageNode, onCreateVideoNode, mentionReferences = [], connectedNodes = [], onDisconnectReference, onStartReferenceSelection, onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
    ...
    const referenceImageCount = mentionReferences.filter((reference) => reference.kind === "image").length;
...
function videoConfigPatch(key: keyof AiConfig, value: string) {
    ...
    if (key === "videoWorkflow") return { videoWorkflow: value };
    if (key === "videoWorkflowSize") return { videoWorkflowSize: value };
