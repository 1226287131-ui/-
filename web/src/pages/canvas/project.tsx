...
function InfiniteCanvasPage() {
    ...
    const completeVideoNodeTask = useCallback(
        async (nodeId: string, config: Parameters<typeof buildGenerationConfig>[0], prompt: string, images: ReferenceImage[], signal: AbortSignal, extra: CanvasNodeData["metadata"] = {}, videos: ReferenceVideo[] = [], audios: ReferenceAudio[] = []) => {
            const resolved = await resolveCanvasVideoReferenceUrls(config.model, { referenceImages: images, referenceVideos: videos, referenceAudios: audios });
            const task = await createVideoGenerationTask(config, prompt, resolved.referenceImages, { signal, videos: resolved.referenceVideos, audios: resolved.referenceAudios });
                    ...
                    await Promise.all(
                        targetIds.map(async (videoId) => {
                            ...
                            try {
                                ...
                                const errorDetails = error instanceof Error ? error.message : t("canvas.projectPage.generationFailed");
                                if (!firstError) firstError = errorDetails;
                                setNodes((prev) => prev.map((node) => (node.id === videoId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails, ...(isVideoTaskFailed(error) ? { videoTaskId: undefined } : {}) } } : node)));
                            } finally {
    ...
    const renderNodePanel = useCallback(
        ...
        [configInputsById, confirmStopGeneration, connectedNodesByNodeId, createImageNodeFromExisting, createVideoNodeFromExisting, disconnectNodeReference, handleConfigNodeChange, handleGenerateNode, handleNodePromptChange, mentionReferencesByNodeId, nodes, renderPluginPanel, runningNodeId, startNodeReferenceSelection],
    );
